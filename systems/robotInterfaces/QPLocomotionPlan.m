        ...
        if supp_idx < length(obj.supports) && obj.support_times(supp_idx+1) - t_plan < obj.EARLY_CONTACT_ALLOWED_TIME
          if ~any(supp.bodies == body_id)
            ...
            if ~isempty(k)
              ...
              qp_input.support_data(end).use_support_surface = obj.supports(supp_idx+1).use_support_surface(j);
              qp_input.support_data(end).support_surface = obj.supports(supp_idx+1).support_surface{j};
            end
          end
        end
      end
